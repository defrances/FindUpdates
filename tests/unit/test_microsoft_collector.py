...
class MicrosoftCollectorTests(unittest.TestCase):
    ...
    def test_sentinel_release_date_uses_revision_history(self) -> None:
        payload = json.loads((FIXTURES / "msrc-cvrf-windows.json").read_text(encoding="utf-8"))
        payload["Vulnerability"][0]["ReleaseDate"] = "0001-01-01T00:00:00"
        payload["Vulnerability"][0]["ReleaseDateSpecified"] = False
        payload["Vulnerability"][0]["RevisionHistory"] = [
            {
                "Number": "1.0",
                "Date": "2026-09-08T07:00:00",
                "Description": {"Value": "Information published."},
            },
            {
                "Number": "2.0",
                "Date": "2026-09-10T07:00:00",
                "Description": {"Value": "Affected software updated with new package information."},
            },
        ]
        collector = MicrosoftCollector(
            client=HttpClient(
        ...
        self.assertEqual(result.metrics.collected, 1)
        advisory = result.advisories[0]
        self.assertEqual(advisory.published_at, datetime(2026, 9, 8, 7, tzinfo=UTC))
        self.assertEqual(advisory.revised_at, datetime(2026, 9, 10, 7, tzinfo=UTC))

    def test_unspecified_sentinel_falls_back_to_catalog_stamp_year_gate(self) -> None:
        payload = json.loads((FIXTURES / "msrc-cvrf-windows.json").read_text(encoding="utf-8"))
        current = payload["Vulnerability"][0]
        current["ReleaseDate"] = "0001-01-01T00:00:00"
        current["ReleaseDateSpecified"] = False
        reprint = json.loads(json.dumps(current))
        reprint["CVE"] = "CVE-2019-0808"
        reprint["Title"] = {"Value": "Win32k Elevation of Privilege Vulnerability"}
        payload["Vulnerability"] = [current, reprint]
        collector = MicrosoftCollector(
            client=HttpClient(
                transport=MappingTransport({}), max_retries=0, min_interval_seconds=0
            ),
