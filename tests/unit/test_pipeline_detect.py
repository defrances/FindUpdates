...
class PipelineDetectTests(unittest.TestCase):
    def test_fixture_detect_analyzes_and_notifies(self) -> None:
        ...
        self.assertEqual(report_json["station_count"], 7)
        self.assertEqual(len(report_json["stations"]), 7)
        self.assertIn("SYNTHETIC-LAB-24H2-01", report_json["stations"])
        self.assertIn("SYNTHETIC-W11-24H2-01", report_json["stations"])
